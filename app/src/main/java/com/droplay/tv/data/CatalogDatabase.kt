...
data class LiveCategoryEntity(val playlistId: String, val categoryId: String, val name: String, val syncVersion: Long)
@Entity(tableName = "vod_categories", primaryKeys = ["playlistId", "categoryId"], indices = [Index("playlistId"), Index(value = ["playlistId", "name"])])
data class VodCategoryEntity(val playlistId: String, val categoryId: String, val name: String, val syncVersion: Long)
...
data class SeriesCategoryEntity(val playlistId: String, val categoryId: String, val name: String, val syncVersion: Long)

@Entity(tableName = "live_streams", primaryKeys = ["playlistId", "streamId"], indices = [Index(value = ["playlistId", "categoryId"]), Index(value = ["playlistId", "normalizedName"]), Index(value = ["playlistId", "addedAt"])])
data class LiveStreamEntity(val playlistId: String, val streamId: String, val categoryId: String, val name: String, val normalizedName: String, val icon: String?, val epgId: String?, val addedAt: Long, val extension: String, val syncVersion: Long)
@Entity(tableName = "vod_streams", primaryKeys = ["playlistId", "streamId"], indices = [Index(value = ["playlistId", "categoryId"]), Index(value = ["playlistId", "normalizedName"]), Index(value = ["playlistId", "addedAt"])])
data class VodStreamEntity(val playlistId: String, val streamId: String, val categoryId: String, val name: String, val normalizedName: String, val icon: String?, val addedAt: Long, val extension: String, val year: Int?, val rating: Double?, val description: String?, val durationMs: Long, val syncVersion: Long)
@Entity(tableName = "series", primaryKeys = ["playlistId", "seriesId"], indices = [Index(value = ["playlistId", "categoryId"]), Index(value = ["playlistId", "normalizedName"]), Index(value = ["playlistId", "addedAt"])])
data class SeriesEntity(val playlistId: String, val seriesId: String, val categoryId: String, val name: String, val normalizedName: String, val cover: String?, val backdrop: String?, val addedAt: Long, val year: Int?, val rating: Double?, val description: String?, val syncVersion: Long)
...
interface CatalogDao {
    ...
    @Query("SELECT * FROM live_streams WHERE playlistId=:playlistId AND normalizedName LIKE '%' || :query || '%' LIMIT :limit OFFSET :offset") suspend fun searchLive(playlistId: String, query: String, limit: Int, offset: Int): List<LiveStreamEntity>
    @Query("SELECT * FROM vod_streams WHERE playlistId=:playlistId AND normalizedName LIKE '%' || :query || '%' LIMIT :limit OFFSET :offset") suspend fun searchVod(playlistId: String, query: String, limit: Int, offset: Int): List<VodStreamEntity>
    @Query("SELECT * FROM series WHERE playlistId=:playlistId AND normalizedName LIKE '%' || :query || '%' LIMIT :limit OFFSET :offset") suspend fun searchSeries(playlistId: String, query: String, limit: Int, offset: Int): List<SeriesEntity>
...
abstract class CatalogDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun get(context: Context): CatalogDatabase = instance ?: synchronized(this) {
            instance ?: Room.databaseBuilder(context.applicationContext, CatalogDatabase::class.java, "droplay-catalog.db").build().also { instance = it }
        }
    }
}
