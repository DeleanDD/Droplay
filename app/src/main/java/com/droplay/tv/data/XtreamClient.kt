        ...
        return buildList {
            ...
            addAll(items("get_series").map { o ->
                ...
                MediaEntry("series:$id", o.optString("name", "Série"), "", MediaKind.SERIES,
                    ...
                    description = o.optString("plot").takeIf(String::isNotBlank), seriesId = id)
            })
        }
    }
    ...
    fun episodes(seriesId: String): List<MediaEntry> {
        ...
        return seasons.keys().asSequence().flatMap { season ->
            ...
            (0 until array.length()).asSequence().map { i ->
                ...
                MediaEntry("episode:$id", "T${season} · ${o.optString("title", "Episódio ${i + 1}")}", "$base/series/$user/$pass/$id.$ext",
                    MediaKind.MOVIE, "Temporada $season", description = o.optJSONObject("info")?.optString("plot"))
            }
        }.toList()
    }
