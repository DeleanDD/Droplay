    ...
    fun history(): List<WatchRecord> {
        ...
        return (0 until json.length()).mapNotNull { i ->
            runCatching { json.getJSONObject(i) }.getOrNull()?.let {
                WatchRecord(it.optString("id"), it.optLong("position"), it.optLong("duration"), it.optLong("at"))
            }
        }
    }

    fun saveProgress(id: String, positionMs: Long, durationMs: Long) {
        if (id.isBlank() || positionMs < 2_000) return
        val records = history().filterNot { it.mediaId == id }.toMutableList()
        records.add(0, WatchRecord(id, positionMs, durationMs, System.currentTimeMillis()))
        val json = JSONArray()
        records.take(60).forEach { json.put(JSONObject().put("id", it.mediaId).put("position", it.positionMs).put("duration", it.durationMs).put("at", it.watchedAt)) }
